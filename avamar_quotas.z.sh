...
# script generates few email templates to be sent to the client if the check backup consumption is larger than the defined quota
...
# this can be put in cron to run every night: (as root)# crontab -u admin -e
...
# just some useful SQL to check the last record of used backup:
# psql -p 5555 mcdb -t -A -F"," -c "select client_name,max(bytes_protected),max(recorded_date) from v_activities_2 where domain= '/ict3' and recorded_date=(select max(recorded_date) from v_activities_2 where domain= '/ict3') group by client_name;"
